...
<<<<<<< HEAD
## Визуализация метрик
Для визуализации метрик удобно применять Grafana. Это ПО визуализирует метрики на основе описанных в json формате `dashboards`. Источником данных `datasource` могут быть разлиные системы, в том числе prometheus. Пользователь взаимодействует с grafanа через браузер, причём отрисовка и, опционально, получение метрик осуществляется браузером. На одном dashboard могут быть показаны метрики из разных datasource. Существует общедоступный каталог dashboards, разделённый по datasource, типам метрик и т.д. Созданный dashboard можно экспортировать/импортировать. А начиная с версии 5.0 dashboard и datasource можно описать в файлах, данные их которых grafana возьмёт при старте. Помимо этого, поддерживается версионирование dashboards.
## Виды метрик
Все метрики можно условно разделить на 3 группы: метрики базовой системы, метрики приложения и метрики бизнес логики. Метрики базовой системы - показатели, предоставляемые операционной системой или системой контейнеризации. В данном проекте для получения этих метрик используется `node-exporter`. Он предоставляет показатели загрузки cpu, использование памяти, диска, сети и т.п.
### Мониторинг docker
Для мониторинга docker engine существуют различные решения, в данном проекте используются следущие решения:
1. cAdvisor - предоставляет большое количество метрик как самого docker engine, так и данных о запущенных контейнерах. Имеет собственный web-интерфейс с простой визуализацией метрик.
2. Docker - сам docker в экспериментальном режиме предоставляет набор метрик для prometheus. Однако набор метрик значительно беднее, чем cAdvisor. К тому же пока документация очень скудна и значение не всех метрик понятно
3. Telegraf - написанный на go коллектор метрик, умеющий собирать метрики из большого количества источников и отдавать их в различном формате. В данном проекте он используется для сбора метрик docker. Количество собираемых метрик несколько меньше, чем cAdvisor, но все базовые есть и они достаточно хорошо документированы.
### Метрики приложения
Метрики такого типа отражают состояние основных показателей приложения. В данном проекте используется метрики "количество запросов" и "время обработки запроса".
### Метрики бизнес логики
Метрики такого типа отражают состояние показателей, важных для бизнеса. В данном проекте используется метрики "количество постов" и "количество комментариев". Часто важно не абсолютное значение таких метрик, а их изменение с течением времени. Для этого удобно применять встроенные в prometheus функции `rate (irate)`.
## Алертинг
Одной из основных задач мониторинга явялется оповещение в случае выхода значения метрики за определённые границы. В состав prometheus входить компонент `alert manager`, ответственный за рассылку уведомления по разлиным каналам: email, slack и т.д. Конфигурация самих `alerts` задаётся в конфигурации prometheus и доступна для обзора черерз web-интерфейс. Alert вызывается, если указанное в `expr` условие истинно более, чем время, указанное в строке `for`. Существует разделение alerts по уровню важности, а также группировка. В данном проекте используется два типа alerts: падение одного из сервисов (метирка `up`) и превышение времени ответа сервиса `ui`. Для того, чтобы ументьшить вероятность ошибочного срабатываения alert, удобно испльзвать 95 перцентиль. Таким образом из рассмотрения изымаются случаные скачки значения искомого показателя.
## Мониторинг облачной инфраструктуры
AWS и GCP имеют свои подсистемы мониторинга, доступ к торым можно получить через API. В данном проекте для получения метрик из GCP, а конкретно из GCE, используется проект `frodenas/stackdriver-exporter`. Он запрашивает указанный список метрик у `Stackdriver` - подсисетмы мониторинга в GCP и отдаёт их в формате, совметимом с prometheus. Таким образом получилось собрать метрики о загрузке cpu, использовании диска и сети. Однако данных об исползовании памяти получить не удалось.
## Запуск проекта
Makefile обновлён с учётом новых docker образов. Большинство образов экспортивано в https://hub.docker.com/u/wildermesser/.
# Централизованное логирование
## Стек технологий
В данном проекте для цетрализованного сбора и обработки логов используется стек EFK - Elasticsearch для хранения и обработки логов, fluentd для сбора логов с сервисов, kibana - интерфейс для просмотра и анализа логов. Так как микросервисы запущены в docker контейнерах, используется logging плагин docker, который отпавляет stdout и stderr во fluentd, который также запущен в контейнере.
## Fluentd
В данном проекте структурированные логи сервиса post и неструктурированные логи сервиса ui обрабатывает fluentd и отсылает их в elasticsearch. Для логов в формате json достаточно описания, а для неструктурированных логов сервиса ui исплользуются несколько grok паттернов, проверяющихся последовательно.
## Распределённый трейсинг
В данном проекте приложения умеют отправлять нужную информацию в сервис `zipkin`. Он также запущен в контейнере и имеет доступ в сеть приложений. Через веб-интерфейс можно посмотреть трейс запросов и увидеть время обработки запроса. Помимо этого, можно увидеть глубину запроса и вовлечённые в обработку сервисы.
